...
def create_transaction_offline(
    ...
    Builds a Tron transaction dictionary for transferring TRX from one address to another using the provided reference block ID for TAPoS metadata. The transaction is not signed and includes all necessary raw data fields for later signing and broadcasting.
...
def create_smart_contract_transaction_offline(
    ...
    """
    Construct an unsigned TRC-20 token transfer transaction offline, encoding the smart contract call and TAPoS metadata.
    ...
    Returns:
        Transaction: A dictionary representing the unsigned transaction, including transaction ID, raw data with encoded contract call, empty signature list, and no permission.
    """
